...
def _normalize_audience(item: dict[str, Any]) -> dict[str, Any]:
...
def list_audiences(
    ...
    """
    if audience_id:
        return [_normalize_audience(client.get(f'lists/{audience_id}', params=api_params))]
    ...
    return [_normalize_audience(item) for item in lists]


def resolve_audience_id(client: MailchimpClient, audience: str, limit: int = 1000) -> str:
    """Resolve an audience name to its Mailchimp list ID."""
    lists = client.get_paginated(
        endpoint='lists',
        items_key='lists',
        limit=limit,
