...
def test_parser_accepts_report_specific_arguments() -> None:
    ...
    campaigns = parser.parse_args(
        ['campaigns', '--audience', 'Customers', '--subject', 'sale']
    )
    contacts = parser.parse_args(
        ['contacts', '--audience', 'Customers', '--email', 'ada']
    )
...
def test_root_help_exits_without_prompt(monkeypatch: pytest.MonkeyPatch) -> None:
